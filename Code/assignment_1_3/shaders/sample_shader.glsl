...
// The maximum amount of ray-marching steps we can take
...
vec2 march_ray(vec2 origin, vec2 direction, float step_size) {
    ...
    for (uint i = 0; i < max_raymarch_iter; ++i) {
        ...
        int shape_index = texelFetch(rasterized_texture, tex_coords, 0).r;
        if (shape_index >= 0) {
...
void main() {
    ...
    // Fetch the shape index from the rasterized texture
    int shape_index = texelFetch(rasterized_texture, tex_coords, 0).r;
    ...
    bool hit = shape_index >= 0 && shape_index < (shape_type == 0 ? circle_count : line_count);
    if (hit) {
        if (shape_type == 0) {
            // Circle logic
            Circle circle = circles[shape_index];
            float distance = length(origin - circle.position);
            float weight = 1.0 / (distance + 1e-5);
            accumulated_color.rgb += circle.color.rgb * weight;
            accumulated_color.a += weight;
        } else if (shape_type == 1) {
            // Line logic
            Line line = lines[shape_index];
            vec2 line_dir = normalize(line.end_point - line.start_point);
            vec2 to_point = origin - line.start_point;
            float cross_product = line_dir.x * to_point.y - line_dir.y * to_point.x;
            vec4 color = cross_product > 0.0 ? line.color_right[0] : line.color_left[0];
            float distance = length(origin - intersection);
            float weight = 1.0 / (distance + 1e-5);
            accumulated_color.rgb += color.rgb * weight;
            accumulated_color.a += weight;
        }
    }
